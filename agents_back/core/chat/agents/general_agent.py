...
<tools>
    <agnt description="system tools that allow you to interact with the application">
        <canvas-show description="Allows graphical display of SVG elements. Use only when the user requests for an svg.">
            <params>
...
class GeneralAgent(AgentBase):
    ...
    async def invoke(self, chat: ChatContext) -> AgentResponse:
        ...
        return AgentResponse(
            Message(type=message_type, src="agent", content=msg, timestamp=datetime.now())
        )